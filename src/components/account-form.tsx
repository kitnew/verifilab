...
export function AccountForm({ projects }: { projects: { id: string; name: string }[] }) {
  ...
  return <form action={action} className="grid gap-5 sm:grid-cols-2">
    ...
    <Field label="Project role"><select className={selectClass} name="role"><option value="AUTHOR">Author</option><option value="REVIEWER">Reviewer</option><option value="CURATOR">Curator</option><option value="OPERATOR">Operator</option></select></Field>
