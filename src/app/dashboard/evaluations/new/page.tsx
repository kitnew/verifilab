...
export default async function NewEvaluationPage({ searchParams }: { searchParams: Promise<{ task?: string }> }) {
  ...
  const tasks = await prisma.task.findMany({ where: { project: { guestWorkspaceId: user?.guestWorkspaceId ?? null } }, select: { id: true, title: true, project: { select: { name: true } } }, orderBy: { updatedAt: "desc" } });
