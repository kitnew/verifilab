...
export default async function EditTaskPage({ params }: { params: Promise<{ projectId: string; taskId: string }> }) {
  ...
  if (!task) notFound();
  if (!actor || !canEditAssignedTask(actor.role, actor.id, task.assignedAuthorId)) return <Card><CardContent className="py-12 text-center"><h1 className="text-xl font-semibold">Author access required</h1><p className="mt-2 text-sm text-slate-500">Only the assigned author, curator or administrator can edit this task.</p></CardContent></Card>;
