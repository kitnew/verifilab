...
export default async function GenerationHistoryPage() {
  ...
  const jobs = await prisma.generationJob.findMany({ where: { project: { guestWorkspaceId: user?.guestWorkspaceId ?? null } }, include: { project: { select: { name: true } }, _count: { select: { tasks: true } } }, orderBy: { createdAt: "desc" }, take: 100 });
